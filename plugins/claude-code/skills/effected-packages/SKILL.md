...
`@effected/*` is an Effect v4-first app kit: 32 packages (31 libraries plus
...
| `@effected/schemastore-cli` | the `schemastore` command over `@effected/schemastore`: `schemastore build` / `schemastore check` a `schemastore.config.ts` (`defineConfig`), a per-schema `published` flag, a drift policy (`--drift`, `--on-drift`, `--force`), JSON output, a GitHub step summary, exit codes 0/1/2/3/64 | publishing SchemaStore documents from a consumer repository without writing a generator script — install as a devDependency beside `@effected/schemastore` (peers; released as a fixed pair) | integrated | [references/schemastore-cli.md](references/schemastore-cli.md) |
