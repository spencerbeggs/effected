...
- **`ConfigFile.Service<Self, A>()(id)`** — a per-schema `Context.Service` class factory you extend for identity; `ConfigFile.layer(TagClass, { schema, codec, resolvers, strategy, validate?, defaultPath?, events?, parseOptions? })` wires it (`schema` is a `Schema.Codec<A, I>`, not a plain `Schema.Schema<A>` — the encoded form `I` matters on the write path). **`parseOptions`** threads `SchemaAST.ParseOptions` into every decode — the field that matters is `onExcessProperty`, which core defaults to `"ignore"`, so unknown keys are dropped silently and a loader can report neither a typo'd section nor a field the schema deliberately removed (a user keeps a stale credential and is told nothing). `validate` cannot substitute: it runs on the DECODED value, after the excess keys are gone. Keys covered by a `Schema.StructWithRest` rest are not excess, so a deliberate pass-through section survives `"error"`. Pair with `errors: "all"`. Available on `ConfigReadOptions` too. Service surface: `load`, `loadFrom(path)`, `loadOrDefault(default)`, `discover` (every source found, in priority order — a corrupt source ABORTS with a typed error rather than being silently skipped), `save` (resolves `defaultPath`, `mkdir -p`s its parent), `write(value, path, options?)` (no directory creation — the path is already vouched for), `encode(value, options?)` (the serialized text WITHOUT writing — byte-for-byte what `write` would put on disk, no event; the `--dry-run` primitive; errors `ConfigEncodeError = ConfigCodecError | ConfigValidationError`, path-less), both taking `ConfigEncodeOptions { header?: string }` — text prepended VERBATIM ahead of the document with exactly one newline (an existing trailing `"\n"` is not doubled); the caller owns its validity in the target format (`#` for TOML/YAML, `//` for JSONC, nothing for JSON), the motivating case being a `#:schema` directive atop a TOML file, `update(fn, default?)` (semaphore-serialized read-modify-write), `validate`.
...
// The salt MUST be cryptographically random, generated once and persisted
// alongside the ciphertext — a constant salt weakens the derived key and
// invites reuse across installations.
