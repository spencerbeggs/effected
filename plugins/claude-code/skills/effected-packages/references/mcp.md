...
| `ToolInputSchema.unknownKeys` / `.formatUnknownKeys` | naming every unknown key in a tool call, without adopting `McpToolkit` |
| `ToolInputSchema.objectRooted` | a tool's parameters are a top-level discriminated union — core dies at boot on the unrewritten union |
  ...
  get_thing: ({ id }) =>
    Effect.fail(
      new NotFound({
        ...
        message: ToolFailure.message(`No thing "${id}".`, { hint: "List the ids first.", suggestedTool: "list_things" }),
      }),
    ),
});

const ToolsLayer = McpToolkit.layer(MyTools).pipe(Layer.provide(MyHandlers));
const Main = ToolsLayer.pipe(Layer.provideMerge(McpStdio.layer({ name: "my-server", version: "1.0.0" })));
...
- **A top-level union parameter dies at boot.** `Tool.make`'s `parameters` must resolve to an object schema for MCP's tool-JSON encoding; a top-level `Schema.Union` of objects is decoded with `orDie` and kills the server at registration. Rewrite it with `ToolInputSchema.objectRooted` before it reaches registration, or design the tool with an object-rooted, discriminated-by-field shape from the start.
