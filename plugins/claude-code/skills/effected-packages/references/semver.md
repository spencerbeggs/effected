...
| `SemVer.parseResult` / `Range.parseResult` / `Comparator.parseResult` | the SAME parse from a synchronous call site — never wrap `parse` in `Effect.runSync` |
...
## Parsing from a synchronous call site

**Every parse here ships a `Result`-returning twin, and it is the primitive — the `Effect` form is derived from it and adds only a tracing span.** Reach for `SemVer.parseResult`, `Range.parseResult` or `Comparator.parseResult` whenever there is no Effect runtime at the call site: a CLI whose house idiom is synchronous `node:fs`, a config file evaluated before any runtime exists, a lint-staged handler.

```ts
import { SemVer } from "@effected/semver";
import { Result } from "effect";

const parsed = SemVer.parseResult(raw);
if (Result.isFailure(parsed)) return null; // parsed.failure is the typed InvalidVersionError
const version = parsed.success;
```

**Never wrap `SemVer.parse` in `Effect.runSync` inside a `try`/`catch`.** It spins up a runtime for a pure string parse and flattens the typed `InvalidVersionError` into a thrown fiber failure that you then re-catch untyped — losing exactly the type information the schema exists to give you. A consumer did this in production because these twins were not documented here; they have shipped since 2026-07-20.

The same policy holds across the kit's pure packages (`jsonc`, `yaml`, `toml`, `glob`, `markdown`, `spdx`, `templates`): grep for `parseResult` before concluding a boundary is Effect-only.
