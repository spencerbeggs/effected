...
| `SchemaLayout` | TypeAlias | Where a versioned document sits relative to its base: `flat` is `<name>-<version>.json` (the only shape SchemaStore serves), `versioned` nests it as `<version>/<name>-<version>.json`. An unversioned document is `<name>.json` under either. | |
