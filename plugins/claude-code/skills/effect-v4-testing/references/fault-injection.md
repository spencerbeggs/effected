...
`FileSystem.layerWith` and no `Layer.mapService` in `effect@4.0.0-rc.109`
(`Layer.ts` / `FileSystem.ts` exports re-checked 2026-08-23) — the scaffold below
...
in one call (`Layer.ts:2063` at rc.109; it is defined as
...
rc.109) builds a service from a partial implementation; any missing member
...
`Layer.fresh(L)` (`Layer.ts:2160` at rc.109) rebuilds with a new `MemoMap`,
