...
description: Use ONLY when the user explicitly asks to bootstrap a fresh copy of the GitHub Action template into their own action — "bootstrap this template", "turn this template into my action", "start a new action from the template". Runs a fixed eight-question interview, writes a plan file, and hands off to designing-an-action. Never load for ordinary action work; for that, start at building-a-github-action.
when_to_use: bootstrap this template, turn this template into my action, start a new action from the template, set up the action template, action template interview, new action from github-action-template
---
