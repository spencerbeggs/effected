...
- **`--force`** is `--drift=allow` for one run, announced loudly; it never overrides a gate failure. Combined with any other explicit `--drift` it is refused as a usage error (`ConflictingFlagsError`, exit `64`) before the config loads — a contradiction, not a precedence question.
