...
| `descend` | Function | Expand a compiled glob pattern against the filesystem under `onUnreadable: "record"`, resolving to a `DescendResult` — the matched FILE paths relative to `cwd` (POSIX separators, sorted) plus the `cwd`-relative path of every mid-walk directory whose `readDirectory` failed for a reason other than `NotFound`. The walk continues past each such directory exactly as `"skip"` does; it never aborts and it never discards the offending path. | expand a compiled glob pattern into files under a directory, prune node_modules |
