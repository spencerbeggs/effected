...
- **Only the carrier declares a bin.** A packed package other than the carrier that declares one of the carrier's bin names fails `BinConflict` before any install: under npm, bun and Yarn's flat layouts it could take the `.bin` slot, and the carrier identity would be lost. Front ends declare no `bin` (see `design-patterns`' `carrier-entry-contract.md`). Only packed packages' `bin` fields are compared: not `directories.bin`, not registry dependencies. `allowSharedBins: true` skips the check for a tool mid-migration off mirror bins; the bins are still verified present but may be a front end's, so assert `binProvenance` meanwhile.
