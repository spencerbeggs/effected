...
- **`@effected/workspaces/testing`** — the repo-shape checks, each a static class. `design-patterns`' `carrier-verification.md` is the why: each check pins one property a carrier package (a CLI and an MCP server sharing an engine) must hold to stay portable and layered correctly.
  ...
  - **`PackedInstall`** — `run(options)` packs the carrier and its closure (`"auto"` = transitive runtime workspace deps), installs it into a scratch consumer per available manager, and returns `PackedInstallResult { consumers: InstalledConsumer[], unavailable, tarballs, scratch }` (`scratch` is the realpath'd scratch root, removed with the scope). `InstalledConsumer.runBin(name, args?, options?)` runs an installed bin and returns `CommandOutput { stdout, stderr, exitCode }` (needs `ChildProcessSpawner`; fails `BinFailed`); `InstalledConsumer.binProvenance(name)` returns `{ package, target }` for a `.bin` symlink or `undefined` for a pnpm shim (needs `FileSystem | Path`); `InstalledConsumer.binPath(name)`; `PackedInstall.timeoutBudget({ managers, installTimeout?, packages, perConsumer? })` returns the `Duration` an outer timeout must cover; `scrubEnv(env)`. Requires `FileSystem | Path | Scope | ChildProcessSpawner | WorkspaceDiscovery`; fails `PackedInstallError` with a `reason`. `packFrom` defaults to `{ directory: "dist/prod/npm/pkg" }`.
...
- **`binProvenance` sees symlinks only.** npm, bun and Yarn (under the `node-modules` linker the run configures) link `.bin` entries, and a dependency's same-named bin can take the slot under their flat layouts; pnpm writes shell shims, and `binProvenance` returns `undefined` for them rather than parsing one. Assert provenance for the linking managers and skip pnpm.
- **`timeoutBudget` needs the package count.** `closure: "auto"` is resolved only inside the run, so pass `packages` (the carrier plus its closure; `result.tarballs` has one entry each). The budget is the worst case of the run's own ceilings, so it runs well above a typical install; set the vitest timeout above it.
