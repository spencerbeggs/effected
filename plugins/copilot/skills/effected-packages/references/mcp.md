...
- **`McpStdio`** — `McpStdio.protocols` is `[McpProtocol.v2026_07_28, v2025_11_25, v2025_06_18]`: stateless first, because real clients (Claude Code 2.1.281, measured) open with `server/discover` rather than `initialize`; never a single entry, since `initialize` only matches stateful adapters; at most one stateless adapter. `McpStdio.layer(options)` → `Layer<McpServer | McpServerClient, never, Stdio>` is `McpServer.layerStdio` with `LogToStderr` **merged** into its own output (`Layer.provideMerge`, so every layer it provides logs to stderr too — a `Layer.mergeAll` sibling is not provided by it and does not inherit the setting) and `Layer.orDie` (a bad `protocols` list is the implementer's own defect). It also reads stdin through a guard that frames it exactly as core's own decoder does: a stdin line that is not JSON is answered on stdout with a JSON-RPC `-32700` parse error and the server keeps serving, instead of core's decoder wedging on that line for every later chunk; a line that is JSON but no JSON-RPC message core can handle (a non-object non-array value such as a bare `null`, an object with a non-string `method` and no usable `id`, or an object with neither `method` nor `id`) is answered `-32600` Invalid Request with `id: null`, where core would otherwise drop the other frames in that chunk or send no reply. Arrays, responses and requests with an `id` go to core, which answers or ignores them itself. **Give each server a fresh layer memo map** — core's stdio protocol layer is a shared constant, so a second `McpStdio.layer` server whose build sees the first one's memo map shares its protocol: only the first server reads stdin, and the second never answers. Merging both into one graph does that, and so does building or providing the second anywhere under the first one's `Effect.provide` — a nested `Layer.build` or `Effect.provide` forks the ambient memo map and does not isolate. Isolate each server with its own `ManagedRuntime`, `Effect.provide(layer, { local: true })`, or its own process. `McpStdio.launch(layer)` → `Effect<never, Error, R>` is `Layer.launch` plus self-reporting: it catches the cause, logs a real failure on stderr itself, and re-raises marked `[Runtime.errorReported] = false` so `runMain`'s own report — which runs outside anything the program provides, and writes through `console.log` when `LogToStderr` was only scoped to the launched effect — never fires a second, malformed report onto the wire. `McpStdio.teardown` maps a success or an interrupt-only exit (stdin EOF) to 0; everything else goes to `Runtime.defaultTeardown`.
...
- **`McpToolkit`** — `McpToolkit.layer(toolkit, options?: McpToolkitOptions)` runs core's `registerToolkit` unchanged, under a registration-scoped decorated `McpServer` whose `addTool` puts an `unknownKeys` pre-check in front of each handler. Rejection stays core's — `Tool.Strict` already decodes with `onExcessProperty: "error"` — but core reports only the *first* excess key; the decorator's pre-check walks the schema the tool actually serves and fails with one `McpSchema.InvalidParams` naming every unknown key path plus the accepted params, before core ever decodes. `options.strict` is `"all"` (default) or `"annotated"`: `"all"` re-annotates every tool without its own `Tool.Strict` annotation to strict; `"annotated"` leaves annotation alone. An explicit annotation always wins in either mode, and a `Tool.dynamic` tool is never re-annotated (core dies at registration on a strict dynamic tool, since it cannot strictly validate a raw JSON Schema). Only a tool that ends up strict gets the pre-check — the same predicate core uses to pick strict decoding. `DEFAULT_STRICT = "all"` because Claude Code sends a tool call's `arguments` with exactly the tool's declared keys and keeps every protocol extra (client info, capabilities, its own tool-use id, the progress token) under the sibling `params._meta`, so strict-by-default rejects nothing a real call sends. The pre-check's rejection is an ordinary `McpSchema.InvalidParams`, so it follows the same per-revision split as core's own (see the Gotchas). Registration goes through core's module-level `McpServer.McpServer.layer`, shared by reference with `McpStdio.layer`'s own copy — never wrap it in `Layer.fresh`.
- **`McpHarness`** (`./testing`) — `McpHarness.make(server, options?)` builds `server` over a queue-backed `Stdio.layerTest` and returns an in-process client: `initialize`/`discover`, `request`/`startRequest`/`notify`, `callTool`, `listTools`, `listResources`, `readResource`, `sendRaw`, `awaitOutboundMethod`, `stderrSoFar`, `consoleLogSoFar`, `close` (`Queue.end`). `listResources` mirrors `listTools` (`resources/list`'s resources, typed `ErrorResponse` on a JSON-RPC error); `readResource(uri)` mirrors `callTool`, returning the whole `resources/read` response. Pass `server` **without** its own `Stdio` — one it provides internally wins and talks to the real terminal, including a `Stdio` that arrives inside a bundled platform layer such as `NodeServices.layer` (`@effect/platform-node`, also bundling `FileSystem`/`Path`/`ChildProcessSpawner`/`Crypto`/`Terminal`): compose the individual layers the server needs and leave `Stdio` out. The server is built with a fresh layer memo map, never the ambient one, so a harness made under another stdio server's `Effect.provide` still serves its own. **Never pass a server layer that provides a layer the test also provides and then reads** — `McpHarness.make(server.pipe(Layer.provide(AppLayer)))` under `Effect.provide(AppLayer)` builds `AppLayer` twice, so the tools write to one instance and the test reads the other; leave the service in the server layer's requirements and provide it once from the test, or build it once and pass `Layer.succeed(Tag, value)`. Responses are matched by id, so notifications interleave freely. Every wait races a stop signal and a corruption signal (`strictStdout`, default `true`, dies the wait on a non-JSON-RPC stdout line), so nothing can hang past the server stopping. On a stateful revision (the default `2025-11-25`), `yield* client.initialize` first: any other request before an `initialize` was sent fails fast with `McpTestFailure` reason `NotInitialized`, never reaching a server that would only answer an opaque refusal: `-32602 Invalid request metadata` when a stateless adapter is listed first, as in `McpStdio.protocols`, or `-32603 Internal error` when only stateful revisions are served; `sendRaw` is never gated.
- **`McpProcess`** (`./testing`) — `McpProcess.spawn(command)` spawns a real child for the current scope and returns `send` (writes one JSON-encoded line), `sendRaw(text: string | Uint8Array)` (writes the given text or bytes verbatim — the only way to exercise `McpStdio.layer`'s stdin guard against genuinely non-JSON input, since `send`/`McpHarness` always encode valid JSON), `nextLine` (fails `StreamEnded` at stream end), `readUntilResponse(id)` (returns `{ response, seen }`, reading past interleaved notifications), `handshake(protocol?)` (**always uses id 1** — a test's own requests start at id 2 or above), `closeStdin` (`Queue.end`, never `shutdown` — an in-flight frame is delivered first), `exitCode`, `stderrSoFar`/`stderrFinal`.
...
- **`InvalidParams` differs per protocol revision — but only for a known tool's own parameters.** When a **known** tool's parameters fail validation, `2024-11-05`, `2025-03-26` and `2025-06-18` answer with a JSON-RPC error, code `-32602`, and `2025-11-25` and the stateless `2026-07-28` with an `isError: true` tool result. `McpToolkit`'s unknown-key pre-check fails with the same `McpSchema.InvalidParams`, so it follows the same split. An **unknown tool** name, or `arguments` that is not an object at all, never reaches a tool's parameter validation and stays a JSON-RPC `-32602` error on every revision. A test that checks only one shape, or asserts `isError` for an unknown-tool call on the newer revisions, misses the other half of the matrix.
- **A top-level union parameter dies at boot.** `Tool.make`'s `parameters` must resolve to an object schema for MCP's tool-JSON encoding; a top-level `Schema.Union` of objects is decoded with `orDie` and kills the server at registration. Design the tool with an object-rooted, discriminated-by-field shape from the start, or build it with `McpToolkit.unionTool` and `McpToolkit.unionHandler`, which serve the union's strict JSON Schema object-rooted and, under `McpToolkit.layer`, reject a bad call as a `Tool.make` decode failure is rejected. When the union is hand-written raw JSON Schema, rewrite it with `ToolInputSchema.objectRooted` and register it as a `Tool.dynamic` tool.
- **`Tool.EmptyParams`, not `Schema.Struct({})`.** A tool with no arguments uses core's own empty-parameters marker. An empty `Schema.Struct({})` dies at registration in either `strict` mode — the same `orDie` decode that kills a top-level union (above): its generated JSON Schema does not satisfy `ToolJson`'s shape, so the server never comes up. It is not a client-side rejection.
