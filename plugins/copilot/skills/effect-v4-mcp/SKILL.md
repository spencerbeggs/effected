...
name: effect-v4-mcp
description: >-
  Use when building, wiring, testing or reviewing an MCP server on Effect v4 — effect/unstable/ai's McpServer,
  Tool and Toolkit plus the @effected/mcp boundary that keeps stdout the JSON-RPC wire and makes tool failures
  readable to an agent. Also triggers on: MCP server, McpServer, Tool.make, Toolkit, layerStdio, tools/call,
  server/discover, initialize, Tool.Strict, unknown keys, isError, structuredContent, InvalidParams, MCP
  resource, mimeType, stdio server exits 130, JSON-RPC on stdout, McpHarness, McpProbe, crash guard.
---
