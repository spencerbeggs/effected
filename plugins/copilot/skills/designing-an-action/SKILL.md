...
description: >-
  Use when designing a NEW GitHub Action, rebuilding or porting an existing action onto the
  @effected suite, or planning any multi-step action build, before writing the spec, the plan, or
  any module. For a single feature added to an existing action, load the matching actions-* skill
  instead; for capability routing, load building-a-github-action.
---
