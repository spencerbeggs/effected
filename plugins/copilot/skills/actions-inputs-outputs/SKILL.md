...
description: >-
  Use when reading a GitHub Action's inputs through ActionInput, writing outputs through ActionOutputs, or
  designing a machine-readable, schema-backed output contract for a workflow or LLM consumer. Also triggers on:
  action input, action output, INPUT_ mangling, ConfigError, dry-run silently defaulted, runner file delimiter,
  setJson, output contract, JSON Schema drift test, ActionInputs service, ActionInputError, schemastore,
  SchemaPipeline, published output schema, versioned schema.
---
...
- **Prove a present-but-malformed input fails through `withDefault`.** `Config.withDefault`/`Config.option` classify a *missing* key as absent, but a present, unparseable value still fails — `Config` tracks input evidence itself, not the issue (`InvalidValue` carries no `actual` field to build a stale "missing" classification from; its constructor is `(annotations?, input?, options?)`). A present-but-malformed value fails even under a default: keep the regression test (`config.pipe(Config.withDefault(x))` fails, not falls back, on malformed input). See `references/input-validation.md`.
