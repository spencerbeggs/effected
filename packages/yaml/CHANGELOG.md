...
Both `push: # only main` and an own-line comment above a value now round-trip byte-intact through a format pass. Previously the two source shapes collapsed to the same AST, so formatting one always rewrote it into the other. [#384][#384] Thanks [@spencerbeggs](https://github.com/spencerbeggs)!
