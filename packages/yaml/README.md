...
One contract to know before pointing this at a repo tree: **the format path is single-document.** When the input is a `---`-separated multi-document stream (a Kubernetes manifest, or the two-document `pnpm-lock.yaml` pnpm 11 writes under `configDependencies`), `YamlFormat.format` returns no edits and `formatToString` returns the input byte-identical — the formatter refuses to touch a stream it cannot fully re-emit rather than truncating it to its first document. Detection is CST-level, so a `---` inside a block scalar or quoted string is content and such files format normally. Validate a whole stream with `Yaml.parseAllResult`, which fails typed when any document carries a fatal diagnostic.
...
- `YamlEdit` / `YamlFormat` — compute byte-minimal edit arrays for formatting and path-based modification, so a change preserves every comment and byte you did not touch; `preserveComments: false` opts out. Single-document by contract: a multi-document stream yields no edits from `format` and a typed `MultiDocumentStream` failure from `modify`, never a truncated stream.
