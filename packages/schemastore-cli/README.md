...
- `check` is the identical walk with no writes: it reports what `build` would do under the same flags and exits the same way, and also fails (exit `1`) whenever a build would write anything — a stale document, a missing one, or a `catalog.json` left behind after the last `catalog` block was removed (reported `orphaned`; `build` reports it too but never deletes it). Run `schemastore build` and commit the result.
