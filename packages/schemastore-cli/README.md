...
`schemas` is keyed by file base name — the key IS the schema's `name`, and `$id`, the write `path` and every catalog URL derive from it, `outputDir` and `baseUrl`; there is no `$id` override. An application that already holds a `HostedSchema` (to write its own `$schema` from) passes it as `hosted` instead of spelling `baseUrl`, `versions`, `current` and `layout` again — the key must then equal `hosted.name`. A typo'd key anywhere in the config is named and rejected rather than ignored. `versions` lists every label the catalog advertises; `current` (default: the newest) is the one generated at `path`/`$id`, and every other label becomes a **frozen** file the CLI verifies but never regenerates — it must exist on disk and declare the `$id` the config derives for it, so advertising a frozen label with nothing on disk, or a file that still carries an old host in `$id` after a `baseUrl` change, fails the build before anything is written (a `baseUrl` change is a re-publish event for every frozen label). `published` (default `false`) marks a version other people already depend on. `baseUrl: "schemastore"` expands `$id` to `https://json.schemastore.org/…` and the catalog URL to `https://www.schemastore.org/…`; any other value is one `https://` base for both. `outputDir` and `onDrift` are top-level only; `baseUrl` and `drift` are top-level defaults an entry may override. `catalog` is required under `baseUrl: "schemastore"` and optional under a custom host. Every schema's declared `catalog` entry lands in ONE file at `catalogPath` (default `<outputDir>/catalog.json`) — never one file per schema.
...
- `check` is the identical walk with no writes: it reports what `build` would do under the same flags and exits under the same conditions. It is the CI gate, so it also fails (exit `1`) whenever a build would write anything — a committed schema or catalog file that differs from what the config generates, or is missing, is stale; run `schemastore build` and commit the result. A `catalog.json` left behind after the last `catalog` block was removed is reported `orphaned` and is stale too — `build` reports it as well but never deletes it (delete it by hand).
