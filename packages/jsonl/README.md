...
- **A cooperating writer's append landing between this journal's own write and its size probe is a real, unclosed race.** The window is two syscalls wide, both taken under this journal's write permit, but a genuinely concurrent external writer can still land inside it. When that happens, this journal's own line's reported offset is wrong by the length of the external write, the interleaved external line is silently dropped rather than ingested, and this journal's own line can be published a second time. No lock-free scheme prevents it: `O_APPEND` never tells a writer where its bytes landed, so only an advisory lock — which the cooperative-writer contract deliberately does not impose — would close it. A read-back verification after every write would detect it, at the cost of one extra read per append, and is declined for that cost. This case is characterized by probe rather than exercised by a test.
...
| Tag | Means | Recovery |
| --- | --- | --- |
| `MalformedLine` | A line is not parseable JSON. Most often a tail caught mid-write. | Read the last valid envelope instead. Check `line.terminated`: `false` means an unterminated fragment — only completion of the **same** interrupted write can make it valid; a later append starts after those partial bytes and cannot repair them, so if the writer never comes back this is a permanent malformed tail and the last valid envelope is the answer. `true` is already a permanent hole. |
