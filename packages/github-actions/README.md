...
- `ActionLogger` — collapsible groups, buffered step transcripts (`withBuffer({ onSuccess: "discard" })` keeps a green log to one line per step, while a failure, defect or interruption still flushes), `withStep(name, effect, options?)` for a summary line on success and a `❌ <name>` header with the full transcript on failure, `::notice::` annotations, and the `Logger` that renders every `Effect.log*` as a workflow command.
