...
describe("VersionDiff", () => {
	describe("between", () => {
		it("classifies by the highest-precedence differing field", () => {
			assert.strictEqual(VersionDiff.between(parse("1.2.3"), parse("2.0.0")).type, "major");
			assert.strictEqual(VersionDiff.between(parse("1.2.3"), parse("1.3.0")).type, "minor");
			assert.strictEqual(VersionDiff.between(parse("1.2.3"), parse("1.2.4")).type, "patch");
			assert.strictEqual(VersionDiff.between(parse("1.2.3-alpha"), parse("1.2.3-beta")).type, "prerelease");
			assert.strictEqual(VersionDiff.between(parse("1.2.3+a"), parse("1.2.3+b")).type, "build");
			assert.strictEqual(VersionDiff.between(parse("1.2.3"), parse("1.2.3")).type, "none");
		});

		it("carries signed numeric deltas and the original versions", () => {
			const diff = VersionDiff.between(parse("2.5.1"), parse("1.0.0"));
	...
	describe("schema", () => {
		it.effect("round-trips through its encoded form with the tag preserved", () =>
			Effect.gen(function* () {
				const diff = VersionDiff.between(parse("1.0.0"), parse("2.0.0"));
	...
	describe("toString", () => {
		it("prints a readable summary", () => {
			assert.strictEqual(VersionDiff.between(parse("1.2.3"), parse("2.0.0")).toString(), "major (1.2.3 → 2.0.0)");
		});
	});
});
