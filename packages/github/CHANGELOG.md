...
- A 422 or 409 whose validation entries carry `code: "already_exists"` now classifies as `alreadyExists` even when no message says so. Creating a release for a tag that already has one returns exactly that shape. It was classified as `rejected`, so `catchIf(GitHubError.hasKind("alreadyExists"), ...)` fallbacks never ran. [#825][#825]
...
| Dependency | Type | Action | From | To |
| --- | --- | --- | --- | --- |
| @effected/github-references | dependency | added | — | 0.0.0 |
