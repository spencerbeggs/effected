...
- `pnpm pnpm:export` → `node allowed-versions.gen.ts && rolldown-pnpm-config export` — regenerate the allowed-versions table (below), then write the catalogs into the root `pnpm-workspace.yaml` and surface drift between the plugin's definitions and what the workspace pins.

Advancing the beta is `pnpm pnpm:up` then `pnpm pnpm:export`.

## The generated allowed-versions table

`savvy.build.ts` carries a `peerDependencyRules.allowedVersions` block between sentinel comments, **generated by `allowed-versions.gen.ts` — never edit it by hand**. One version-qualified rule per v4 lock-catalog package (`"<satellite>@<its pin>>effect"`, valued at the effect pin) retires the recurring warning class where an `@effect/*` satellite sits at a different beta than the installed `effect`. The table must stay pure literals: `rolldown-pnpm-config export` statically evaluates the config source and rejects computed values. The version qualifier keeps a same-named v3 satellite's genuine unmet peer warning alive, and the kit's own `@effected/*` artifacts are deliberately not covered — their post-advance stranding is repaired by the toolchain republish cycle. The drift tripwire in `__test__/allowed-versions.test.ts` fails whenever the committed table differs from regeneration; running the generator (or `pnpm pnpm:export`) is the fix, never a hand edit.
...
The v3/v4 peer-resolution defect is fixed in pnpm 11.12.0; there is no expected-residual set to ignore. The `rolldown-pnpm-config` v3-satellite warnings this section once tracked were **resolved by the 2026-07-18 toolchain update**. `pnpm peers check`'s one current known issue is the published-exact-pin class a catalog advance produces — a registry `@effected` artifact peering on the previous exact beta; the live occupant is recorded in the root `CLAUDE.md` Dependencies paragraph, the resolution history in [effect-standards.md](../../.claude/design/effected/effect-standards.md#open-defect-one-peers-check-issue), and the proposed structural remedy (a `peerDependencyRules.allowedVersions` table generated by `pnpm pnpm:export`) in [pnpm-plugin-effect.md](../../.claude/design/effected/packages/pnpm-plugin-effect.md). Do not silence it or treat it as license to tolerate a second: any other peer warning is a genuine closure defect to fix upstream.
