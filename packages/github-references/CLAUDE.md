...
GitHub's issue-reference grammar as pure functions: the nine closing keywords and three dialects, strings in and values out — no service, no layer, no client. Extracted from `@effected/github` on 2026-08-17 and pending its first release. 3 `src/` files, 2 test files, 25 tests.
...
- **Every pattern derives from the keyword constants** rather than spelling them again, so a keyword added to either set cannot drift from the grammar that reads it; `closing` is membership in `CLOSING_KEYWORDS`, widened once so no call site casts.
