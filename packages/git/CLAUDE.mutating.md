...
- **`fetchUnshallow` is a distinct mode, and the CALLER guards.** git rejects `--unshallow` in a non-shallow repo and the method does not tolerate that (tolerating would swallow every other fetch failure shape) — probe with `isShallow` first.
