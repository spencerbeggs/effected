...
- **The ssh pin is APPENDED to what git would have used, never substituted for it — and the resolution is per-`cwd`, not per-service.** `GIT_SSH_COMMAND` beats `core.sshCommand` and `GIT_SSH` in git's own precedence order, so pinning a bare `ssh` silently discards a `core.sshCommand` deploy key and turns "prompts for a passphrase" into "cannot reach the remote at all" (verified against git 2.55). So before a network member spawns, `resolveSshEnv` resolves what git would have used — `GIT_SSH_COMMAND` from the environment, else `core.sshCommand` at THAT `cwd`, else plain `ssh` — and appends to it. The config read is per-call because `core.sshCommand` can be repository-local while one `Git` instance serves every `cwd`; it is skipped when the environment variable already decides, and it deliberately runs through `runFor` (base env, no ssh pin) because routing it through the network path would recurse.
  - **`withBatchMode` DECLINES in two cases, and both are load-bearing.** The program's basename must be `ssh`, mirroring git's own variant detection — `plink` has no `-o KEY=VALUE` form (its switch is `-batch`), so appending breaks a working PuTTY setup outright instead of degrading it. And a command that already mentions `BatchMode` is left alone, because OpenSSH takes the FIRST value obtained for a repeated option (verified with `ssh -G` against OpenSSH 10.3), so appending after a caller's `BatchMode=no` is inert — the caller's decision stands rather than being papered over with a pin that does nothing.
