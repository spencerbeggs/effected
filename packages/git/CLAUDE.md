...
- **`LC_ALL=C` + `GIT_TERMINAL_PROMPT=0` + `extendEnv: true` are pinned on every `GitCommand`, unconditionally.** Classification depends on stable, untranslated stderr text (`"not a git repository"`, `"unknown revision"`); a localized message silently misclassifies into `GitCommandError` instead of the typed domain error. `GIT_TERMINAL_PROMPT=0` keeps a network-touching member (`lsRemote`, `fetch`, `push`, `pull`, `submoduleAdd`) against a credential-requiring remote failing fast on git's auth error instead of blocking on a prompt until the 30s `GIT_TIMEOUT` fires. `extendEnv: true` is required alongside them because its default is owned by whichever platform backend implements `ChildProcessSpawner`, not by core — and because a pinned key wins over the merged parent environment, `GIT_TERMINAL_PROMPT=1` in `process.env` does NOT re-enable prompting. **There is no opt-out through the `Git` service**, which builds and spawns internally; a caller that genuinely wants an interactive prompt must take the `GitCommand` value and override the key with `ChildProcess.setEnv` before running it itself. `GIT_TERMINAL_PROMPT=0` gags git's own prompt only — `ssh`'s passphrase and host-key prompts read `/dev/tty` directly and are unaffected — #670.
