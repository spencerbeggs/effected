...
- **The environment pins live on the `Git` SERVICE, not on `GitCommand`.** A `GitCommand` constructor is a pure, context-free value carrying argv and a redaction mask and NOTHING else — no `cwd`, no `env`. `Git` applies both at its single spawn choke point (`runClassified`), via `ChildProcess.setCwd` and `ChildProcess.setEnv`. This is deliberate and was moved here in #670: the pins exist to serve `classify` and `GIT_TIMEOUT`, which live in `Git.ts`, and one of them must be computed from the caller's own environment, which a pure constructor must never read. Do not "restore" a pin onto `GitCommand` — `assertGitCommand` asserts `options.env` is `undefined` across all 81 constructor cases and will fail at once.
  ...
  - `GIT_SSH_COMMAND` — **the one conditional pin.** `ssh` reads a key passphrase and a host-key confirmation from `/dev/tty` directly, so no askpass or terminal-prompt pin reaches them; `-o BatchMode=yes` is the only lever that makes `ssh` fail instead of block (probed under a real pty: without it, first contact with an unknown host hangs indefinitely). But `GIT_SSH_COMMAND` is a variable users legitimately set — a custom identity, a jump host, a corporate wrapper — and a pinned key WINS over the merged parent environment, so a hard pin would turn "prompts for a passphrase" into "cannot reach the remote at all". The caller's value is therefore **preserved and appended to**; only an absent or blank value is replaced wholesale.
