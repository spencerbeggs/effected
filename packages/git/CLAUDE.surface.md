...
- `GitCommand.ts` — 69 pure constructors returning `GitInvocation` values (a core `ChildProcess.StandardCommand` plus the redacted argv). Every one is cwd-less AND env-less: the private `git` helper sets `{ extendEnv: true }` and nothing else. `Git` applies both `cwd` and the environment pins per call, via `ChildProcess.setCwd` and `ChildProcess.setEnv` — each dual, each returning a **new** command and leaving the pure value untouched (#670).
