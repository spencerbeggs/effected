...
- `GitCommand.ts` — 69 pure constructors returning `GitInvocation` values (a core `ChildProcess.StandardCommand` plus the redacted argv). Every one is cwd-less: the private `git` helper pins `{ env: { LC_ALL: "C", GIT_TERMINAL_PROMPT: "0" }, extendEnv: true }` and nothing else. `Git` applies `cwd` per call via `ChildProcess.setCwd`, which is dual and returns a **new** command, leaving the pure value untouched.
