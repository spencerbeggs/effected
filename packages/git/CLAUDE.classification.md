...
`"quiet"` backs `defaultBranch`, `configGet`, `configGetAll` (unset key → `[]`) and `checkIgnore` (`git check-ignore` exits 1 silently when NO path is ignored → `[]`). Each runs with `--quiet` or relies on a silent exit 1 meaning "unset", so any exit-1 WITH stderr text is a real failure, not an absence. `"noSuchRemote"` backs `remoteUrl`: `git remote get-url` prints `"No such remote '<name>'"`, which degrades to `Option.none()` rather than `GitCommandError`.
