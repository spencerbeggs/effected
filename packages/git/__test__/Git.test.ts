...
/**
 * The argv of the `core.sshCommand` probe a network-touching member runs
 * before its own invocation, to resolve the ssh command git would have used.
 */
const SSH_PROBE_ARGS = ["config", "--get", "core.sshCommand"];

const isSshProbe = (args: ReadonlyArray<string>): boolean =>
	args.length === SSH_PROBE_ARGS.length && args.every((arg, index) => arg === SSH_PROBE_ARGS[index]);
...
describe("Git — remaining tiers (round 2)", () => {
	...
	describe("spawn environment (#647, #670)", () => {
		 ...
		 *
		 * `env` seeds the ambient ConfigProvider; `configured` is what the probe
		 * reports for `core.sshCommand` (omitted = unset, a silent exit 1).
		 */
			...
			Effect.gen(function* () {
				...
				const recording = Layer.succeed(
					...
					ChildProcessSpawner.make((command) => {
						...
						const unset = probe && options.configured === undefined;
						return Effect.succeed(
							ChildProcessSpawner.makeHandle({
								...
								kill: () => Effect.void,
								stdin: Sink.drain,
								stdout: Stream.make(new TextEncoder().encode(probe ? `${options.configured ?? ""}\n` : "abc123\n")),
								stderr: Stream.empty,
								all: Stream.empty,
		...
		it.effect("preserves a caller's GIT_SSH_COMMAND and appends BatchMode to it", () =>
			Effect.gen(function* () {
				...
				// The environment variable already settles it — git resolves it ahead
				// of core.sshCommand — so the probe is skipped entirely.
				assert.isFalse(seen.some((spawn) => isSshProbe(spawn.args)));
			}),
		);
