...
- `ConfigDependencyHooks.ts` — `ConfigDependencyHooks` contract, `HookInjection`, and three layers: `layerNoop` / `layerLive` / `layerSubprocess` (the last requires core's `ChildProcessSpawner`)
...
**No composite provides `PublishabilityDetector` — and none requires it either.** Nothing inside `Workspaces.layer`, `layerWithGit` or `layerWithConfigDependencies` consumes a detector, so their `R` stays `FileSystem | Path`. The requirement surfaces in the `R` of each **operation** that asks a publishability question (`VersioningStrategy.detect`), which is where unwired programs fail to compile — possibly far from the layer-wiring site. The common case is one explicit merge (NOT `Layer.provide` onto the composite — the composite doesn't require the detector, so `provide` would discard it and it would never reach the program's `R`):
...
`directory` is the resolved **workspace root**, not the caller's cwd — resolving that root is why this layer exists rather than `LocalExec.layerFor`. And the argv prefixes come from `LocalExec.prefixes(name)` — all three of them, `prefix` / `dlxPrefix` / `scriptPrefix` (the last one required on `ExecContext`, so this layer cannot forget to pass it on): **never hard-code an exec, dlx or script-runner prefix here**, the table lives in `commands` and the tests assert against it so the two cannot drift.
...
A pnpm config dependency's pnpmfile `updateConfig` hook can mutate catalogs; replaying it executes config-dependency code, so it is gated behind **three** layers. `layerNoop` returns `{ catalogs: seed, releaseAge: {} }` untouched; `layerLive` dynamically `import()`s each pnpmfile **in process** and replays it; `layerSubprocess` performs the same replay in a `node` child process and therefore requires core's `ChildProcessSpawner` in `R`. Composite mapping: `layer` → `layerNoop`, `layerWithConfigDependencies` → `layerLive`, `layerWithConfigDependenciesSubprocess` → `layerSubprocess` (on both `WorkspaceCatalogs` and `Workspaces`).
...
The pnpmfile is loaded by trying `pnpmfile.mjs` **first** (pnpm 11 ships the config-dependency pnpmfile as an ES module — a pnpm-11-native config dep may carry only `.mjs`) and falling back to `pnpmfile.cjs` (legacy); `import()` loads both (both layers). The "no pnpmfile" skip is keyed on the dynamic `import()` raising `ERR_MODULE_NOT_FOUND` **for the candidate file itself** — discriminated by comparing Node's `err.url` (the offending module's URL) against the candidate URL, so an `ERR_MODULE_NOT_FOUND` for a module the pnpmfile *imports* surfaces typed rather than being mistaken for an absent file. There is **no `existsSync` precheck** (it returns false for an existing-but-inaccessible file and would silently skip a real hook); any other load failure surfaces typed. `internal/catalogs.ts` stays the only `@pnpm/catalogs.*` importer.

### `layerSubprocess` exists because a BUNDLER cannot see through a computed import

`layerLive` computes its `import()` target at runtime, and a bundler (rspack) compiles a computed dynamic import into a context module that throws `Cannot find module 'file:///…'` at runtime — so `releaseAgeGate()`, the reason an Action opts into hooks at all, was uncallable from any bundled GitHub Action (#280, a silk-update-action dogfood request). `layerSubprocess` moves the computed load out of the bundle graph. Four things about it are load-bearing:

- **The replay program is a STATIC string constant and the runtime values travel via argv** — `node --input-type=module -e <script> <root> <seedJSON> <...names>`, read back as `process.argv.slice(1)`. Interpolating any runtime value into the script text puts it back in front of the bundler and reinstates the bug. `ChildProcess.make` spawns without a shell, so argv is argv.
- **Typed-semantics parity with `layerLive` is the contract**, pinned by an integration test driving both layers against one on-disk fixture: `.mjs`-before-`.cjs`, the `err.url`-equality skip discrimination, the same hook-locator shapes, the same synchronous hook call, the same tolerant threading, the same per-dependency `"hooks"`-source error attribution, and the same `..`-segment refusal — validated **before** spawning, so no subprocess ever sees a traversal name. Change one side and change the other.
- **The child's protocol payload is framed by `@effected/commands`' `Run.jsonLine`** against a strict `{ ok: true, config } | { ok: false, name?, message?, stack? }` schema — last non-empty stdout line, parsed regardless of exit code, so a hook's own `console.log` noise is tolerated and the envelope discriminates success in-band. The hand-rolled copy of that framing was deleted; do not reintroduce it here.
