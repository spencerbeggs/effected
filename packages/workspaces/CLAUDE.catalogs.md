...
Each rules axis threads **independently**, so a malformed `ignoreMissing` cannot blank a well-formed `allowedVersions`. `ignoreMissing` and `allowAny` are **carried but unconsumed** — separate suppression axes nobody has measured, and an unmeasured suppression is what produced the false positives this seam exists to remove. Only `allowedVersions` is consumed, and `allowedVersions` keys appear in **two spellings** that both must be handled: parent-versioned from the workspace file (`"@effect/ai-anthropic@4.0.0-rc.109>effect"`) and unversioned from a plugin (`"@effect/vitest>vitest"`).
...
- **The rules seed rides argv, in its own slot** (`root, seedJson, rulesJson, ...names`) — never spliced into the program text, which must stay a static string for the same reason the root and seed do.
