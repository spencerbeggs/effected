...
Memoization is per **resolved** root (so sibling directories in one workspace share one discovery), in a map deliberately separate from the layer-bound memo — folding them together would make every layer-bound call re-run the root ascent. It grows one entry per distinct root, which is the point for a multi-worktree host; `refresh()` clears all of them plus the layer-bound one, and `refreshIn(directory)` drops exactly one so refreshing a changed worktree does not discard siblings that did not change. `refreshIn` invalidates the cell **before** dropping the reference — a fiber already holding that memo keeps its own reference, and leaving the cached value live would let it replay a discovery the call was asked to discard. All three per-root methods fail typed on a directory in no workspace, so a bad path answers the same way everywhere; a caller preferring a no-op writes `Effect.ignore`.
