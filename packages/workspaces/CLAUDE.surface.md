...
`Workspaces.ts` exposes `layer`, `layerWithConfigDependencies`, `layerWithConfigDependenciesSubprocess`, `layerWithGit`, `layerWithGitAndConfigDependencies`, `layerWithGitAndConfigDependenciesSubprocess`, `layerWithGitAndHooks`, `resolvers`, the one-call manifest path (`resolverLayer`, `resolveManifest`) and `localExecLayer`.

**The matrix is git-tier × catalogs-layer, and all six cells now exist.** `layerWithGit` wires the NO-OP catalogs layer; wanting snapshots + change detection + hook replay used to mean hand-rebuilding the whole graph, and a downstream copy of it rotted against the composite it duplicated. Every git composite is one `withGit(hooks, options)` — public as `Workspaces.layerWithGitAndHooks`, the way a test reaches `ConfigDependencyHooks.layerFrom` through the real graph — which builds the core over the same hooks reference itself, so the three fixed cells cannot drift and the hooks/core pairing is structural. **On the git composites the subprocess variant costs nothing** — its `ChildProcessSpawner` is already required for `Git` — so a bundled consumer should just take `layerWithGitAndConfigDependenciesSubprocess`. The git composites take `WorkspacesGitOptions` (`WorkspacesOptions` + `WorkspaceSnapshotsOptions`), which is how `seedCatalogs` reaches `at(ref)` without hand-composing.
...
**Related:** [discovery](./CLAUDE.discovery.md) · [catalogs](./CLAUDE.catalogs.md) · [snapshots](./CLAUDE.snapshots.md) · [peers](./CLAUDE.peers.md)
