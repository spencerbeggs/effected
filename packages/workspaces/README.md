...
Only the carrier may declare its bins. If any other packed package declares one of the carrier's bin names, the run fails `BinConflict` before installing: under npm, bun and Yarn's flat layouts either package can take the `.bin` slot, so the bin check and every bin run could pass on the wrong one. The check reads the `bin` field of the packed packages only (the carrier, the closure and `overrides`): it does not read `directories.bin`, and a registry dependency declaring the same name goes undetected. A tool still migrating its front ends off mirror bins can pass `allowSharedBins: true` to skip the check; the expected bins are still verified present, but they may belong to a front end, so assert `binProvenance` under the linking managers until the migration lands.
