...
Node reads the unauthenticated nodejs.org index and `nodejs/Release` schedule, so it needs only an `HttpClient`; Bun and Deno read GitHub releases through `GitHubClient`, with anonymous, explicit-token (`GitHubAuth.token`) and environment-detected (`GitHubClient.layerDefault`) auth. Tagged errors throughout — `NoMatchingVersionError`, `FreshnessError`, `RateLimitError` and the rest — each carry their cause structurally.
