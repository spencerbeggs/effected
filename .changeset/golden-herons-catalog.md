...
* **`CanonicalJson`** — deterministic serialization (insertion-order keys, single trailing newline) with typed failures instead of `JSON.stringify`'s silent drops of `undefined`/`NaN`/non-plain values.
