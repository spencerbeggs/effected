...
Typed failures everywhere: a malformed file is a `TsconfigParseError`, a broken chain is a `TsconfigExtendsError` with a `not-found` / `cycle` / `depth` / `empty` reason, and IO errors flow through as `PlatformError`. Nothing fails as a defect.
