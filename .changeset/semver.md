...
Seven tagged errors — `InvalidVersionError`, `InvalidComparatorError`, `InvalidRangeError`, `UnsatisfiableConstraintError`, `EmptyCacheError`, `VersionNotFoundError`, `UnsatisfiedRangeError` — each carry the structured payload a caller needs to report what actually went wrong.
