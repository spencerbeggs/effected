...
`@effected/config-file` is an **optional** peer, consumed only by `ConfigIssueRenderer`, which is a module nothing else imports — so a consumer who does not install it never reaches for it at runtime.
