...
- `plugin/` — "effected", a Claude Code plugin (28 skills, 4 agents) dogfooded during package work; in development.
...
- `schemastore` — Effect Schemas published as SchemaStore-shaped Draft-07 JSON Schema documents: `StoreDocument` assembly (owning the `#/$defs` restore the core lowering undoes), versioned and unversioned catalog modes over a store-native label grammar, fileMatch hygiene lint, `DocumentLint`, language-server annotation carriers re-grafted after the lowering, a `SchemaValidator` contract seam the consumer closes with ajv, and `SchemaFile` write-if-changed IO; annotate at the definition site — a usage-site annotation on a hoisted schema carries nothing (boundary).
