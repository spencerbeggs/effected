...
**`pnpm peers check` reports exactly one known issue**: the registry-published `@effected/jsonc@0.2.0` (inside the `@savvy-web` toolchain graph) peers on exactly `effect 4.0.0-beta.98` while the catalog now installs `4.0.0-beta.99` — the flip side of the lock strategy's exact peer pins: any catalog advance strands previously-published kit artifacts until the toolchain chain republishes (2026-07-19 beta.99 advance). A generated `peerDependencyRules.allowedVersions` table in `pnpm-plugin-effect` is under consideration to retire this warning class entirely. Do not silence it or tolerate a second: **any other warning is a genuine closure defect to fix upstream.**
