...
The `effect` / `effectPeers` catalogs this package defines are the mechanism behind the [peer-dependency discipline](../effect-standards.md#peer-dependency-discipline) in the standards: `@effected/*` libraries keep `effect` as a `catalog:effect` peer, and the interim pnpm-resolver configuration (`autoInstallPeers`, `dedupePeerDependents: false`, the `@effect/tsgo`-as-typechecker choice) that keeps the v4 beta from poisoning the v3 silk toolchain all operate over the catalogs generated here. Advancing the beta is a single `pnpm pnpm:up` + `pnpm pnpm:export`.
