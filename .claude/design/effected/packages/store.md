...
**Re-raising an interrupt has a trap of its own, and it is not `Cache`-specific.** `Layer` has no `failCause` on this release line, so the only way to fail a layer is an effect inside one, and the obvious spelling — `Layer.effectContext(Effect.interrupt)` — reports *this* fiber as the interruptor, discarding the fiber that actually cancelled the work. **`Effect.interrupt` misattributes; it does not erase**, and misattribution is the harder failure to notice — an empty interruptor set reads as "no attribution available", a populated one reads as fact. The combinator rebuilds the cause from `Cause.interruptors`, which preserves the original interruptor (the first, where the source is a set — this release line offers no multi-interruptor constructor) and stays `Cause<never>`, so the `never` error channel survives. The mutation-testing corollary: a mutant of this guard has to be the mistake a maintainer would actually make (`Effect.interrupt`), not a construct production code never had (`Cause.interrupt()`), or it discriminates against nothing. Any future general `Layer.degrading` sibling inherits this verbatim.
