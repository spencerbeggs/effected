...
**The sync hashing twins bind the platform in the caller, not in the package.** `hashResult`/`hashTextResult` take a `JsoncDigest` — `(bytes: Uint8Array) => Uint8Array` — and return a `Result`, so a consumer inside a synchronous host hook (a bundler plugin's `preprocess`, a cache `read`/`write` invoked with no fiber to run an `Effect` in) can fingerprint at all. The shape is `@effected/tsconfig-json`'s `TsconfigLoaderSyncOptions` shape: the package still imports nothing from `node:*` and assumes no runtime, and a Node consumer passes a one-line `createHash("sha256")` wrapper. They are named `*Result`, not `*Sync`, per the [sync primitive policy](../sync-primitive-policy.md#naming-result-never-sync) — what differs is the return type.

A caller-supplied digest can fail two ways a `Crypto` layer cannot, and **both must arrive as a `Result`** — these twins exist to be called from a synchronous host hook, and either one escaping would crash that host over a platform-binding mistake. The digest can **throw** (an algorithm name OpenSSL does not know), so the call is wrapped exactly as `TsconfigLoaderSync` wraps the consumer's `readFile`; and it can return the wrong **width**, so the 32 bytes are checked rather than emitting a plausible-looking digest of the wrong algorithm (a SHA-1 binding would otherwise have produced 40 hex characters). Both fail typed with `InvalidDigest` at path `""`. No check can catch a *different* 32-byte algorithm; that much is documented on `JsoncDigest` and is the caller's to get right, exactly as Windows correctness is the caller's in `TsconfigLoaderSync`.

The output format is a cross-package guarantee: exactly 64 lowercase hex characters, no `sha256:` or other algorithm prefix — the same bare-hex vocabulary `@effected/sbom`'s `Sha256Digest` schema decodes, so a fingerprint flows into an attestation subject downstream **without this package taking a dependency edge on `sbom`**. `hashText`/`hashTextResult` additionally expose `normalizeEol` (also available standalone as the pure `JsoncFingerprint.normalizeEol`) for file content that must fingerprint identically across checkout line-ending settings.
