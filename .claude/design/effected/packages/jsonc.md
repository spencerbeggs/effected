...
Status: **merged.** The four review-flagged decisions were resolved (recorded below with rationale), the carry-forward learnings from the semver migration are baked in as house policy, and the port landed with all gates green (tests, typecheck, biome, zero-warning `dist/prod/issues.json`). Two post-merge tickets have since closed against it — the `parseTree` depth blowup (#13) and the duplicated bracket-balancing skips (#15) — both recorded in [input hardening](#input-hardening-as-built). This doc records the *as-built* design; per the semver precedent it is promoted to `current` with a raised completeness and inline "As-built:" deviation notes woven into the sections below. `@effected/jsonc` is also the reference template for the `@effected/yaml` port that follows it — the parity convention in [jsonc/yaml parity](#jsoncyaml-parity-convention) is a migration requirement, not a nicety.
...
As-built: the recursive class originally carried a `@public JsoncNode_base` const whose factory-return-type annotation used `Schema.Schema<JsoncNode>` (annotating `children` with `typeof JsoncNode` is TS2506, and `Schema.Codec<Self>`-style forms fail because the node's `Encoded` differs from its `Type`). As-built (realignment, 2026-07-08): the base const is gone — the class is written **inline** with the synthesized `_base` symbol suppressed in `savvy.build.ts`. Only the `Schema.suspend` callback's own return-type annotation (`Schema.suspend((): Schema.Schema<JsoncNode> => JsoncNode)`) survives, since a recursive `suspend` still needs its explicit return type; the whole-class factory-return annotation is no longer needed. The `effect-api-extractor-bases` skill records the inline recursive-class idiom.
...
| `JsoncModificationError` | `JsoncModifier.modify` (navigation miss / invalid edit) | `path: JsoncPath`, `expected: "object" \| "array"`, `depth: number`, `offset?: number` |
...
As-built: the firewall landed exactly as designed. `src/internal/parser.ts` owns the `JSONC_PARSE_ERROR_CODES` const and returns raw `{ code, offset, length }` records; `Jsonc.ts` builds the `@public JsoncParseErrorCode` schema *from* that const and computes each detail's `line`/`character` from `offset` against the source text. Because offsets are the parser's single positional currency, the scanner dropped its own `line`/`character` tracking accordingly. One wrinkle: `JsoncModificationError.offset` exists in the payload (per the table above) but is currently **unpopulated** — modify mismatches carry `path` + `expected` + `depth` and leave `offset` absent; populating it is deferred until a consumer needs positional modify errors.

As-built (realignment, 2026-07-08): `JsoncModificationError` was restructured from the transitional `reason: string` field to typed `expected` (`"object" | "array"` — the container kind the failing segment required) and `depth` (1-based index into `path` where navigation failed) fields, with `message` computed via getter from them. This brings it under the structure-preserving-errors house rule (`YamlModificationError` was already compliant); the fields differ from yaml's because the underlying failures differ — the jsonc/yaml parity convention binds `Edit`/`Range`/`Path`, not this error.
...
Unlike yaml's two-stage CST/composer shape, jsonc's recursion is spread across **five independent surfaces**, each guarded separately: the recursive-descent parser's value and tree modes (a new `NestingDepthExceeded` code in `JsoncParseErrorCode`, one deduped fatal diagnostic; over-deep containers are consumed iteratively via bracket-counting so recovery still makes progress), `JsoncNode.toValue`/`findAtOffset`/`buildPath`, `Jsonc.equals`/`equalsValue`'s structural walk (over-deep comparison returns `false`), the `JsoncVisitor` SAX walk (in-band error event) and `internal/navigate.ts#skipValue` (rewritten iteratively).

The bracket-counting skip itself — plus the malformed-closer guard — has **one implementation**, `skipBalancedValue` in `src/internal/skip.ts`, parameterized over a token cursor so the parser, the modifier's navigation and the visitor each keep their own advance discipline over a shared algorithm (issue #15, closed). Future recursion-hardening fixes land in one place.

**Tree construction is validation-free (issue #13, closed).** `Jsonc.parseTree` was exponential in nesting depth because `Schema.Class` construction re-parses the recursive `children` field and each element parse re-runs the class transformation, which re-parses *its* children. `disableChecks` does not help — it skips filter checks, not the structural parse. The parser now builds nodes through `makeNodeUnsafe` in `src/JsoncNode.ts`, an internal path (never re-exported from `index.ts`) that assigns props onto the class prototype exactly as the `Data.Class` base constructor would; the parser guarantees validity by construction, since every field comes straight off a scanner token. Public `JsoncNode.make` and `new` stay fully validating, and the `treeOverflow` empty-children workaround this section previously described is **retired** — an over-deep `parseTree` now builds the real capped-depth tree in milliseconds and still records its fatal diagnostic. The one contract the unsafe path carries: absent optional fields must be *omitted*, never passed as an explicit `undefined`, which `make` would have rejected.
...
- **All public `*Base` export pairs** (`JsoncParseErrorBase`, `JsoncNodeNotFoundErrorBase`, `JsoncModificationErrorBase`) — v3's doubled public surface stays banned. The API-Extractor base need is met by the inline factory + narrow `_base` suppression (see the [API Extractor bases](#api-extractor-bases-house-policy) as-built note): no `*_base` symbol is exported at all.
