...
The one thing a caller-supplied digest can break that a `Crypto` layer cannot is the **width** of the answer, so the twins check it: a return that is not 32 bytes fails typed with `InvalidDigest` at path `""` rather than emitting a plausible-looking digest of the wrong algorithm (a SHA-1 binding would otherwise have produced 40 hex characters). No check can catch a *different* 32-byte algorithm; that much is documented on `JsoncDigest` and is the caller's to get right, exactly as Windows correctness is the caller's in `TsconfigLoaderSync`.
