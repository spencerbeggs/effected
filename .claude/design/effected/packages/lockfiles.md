...
Its consumer is the future `@effected/workspaces`, whose `LockfileReader` service (root find + PM detect + file read + dispatch) stays there and consumes this package. Like `@effected/glob`, lockfiles lands before its consumer exists — it is on the [release gate](../releases.md#the-gate) because `workspaces` reads lockfiles and `vitest-agent` consumes workspaces transitively.
...
- `Lockfile.parse(content, { format }): Effect<Lockfile, LockfileParseError>` — the package's only fallible boundary, wrapped in `Effect.fn("Lockfile.parse")`, dispatching to the internal per-format transform.
...
When `@effected/workspaces` lands, its `LockfileReader` service re-expresses as: find root, detect PM, read file (`LockfileReadError` stays there), `Lockfile.parse` — then for pnpm, read the workspace manifests it already knows how to read and apply `withImporterNames`; `checkIntegrity` becomes manifest IO + `LockfileIntegrity.check`; `resolvedVersion` becomes `packagesNamed(name)[0]` with no request cache; `CatalogSet.fromLockfileCatalogs` types against `PnpmCatalogs`. The `PackageManager` literal in workspaces either aliases `LockfileFormat` or maps to it at the reader boundary — decided at the workspaces port.
