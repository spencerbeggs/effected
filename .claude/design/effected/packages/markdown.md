...
**Frontmatter composition helper.** A `MarkdownFrontmatter.resolved` one-call composition (parse → extract → resolve → validate) is unspecified and was deliberately left unbuilt in P3; decide at P6/adoption whether it lands or stays out.

**jsonc stringify parity.** The P3 frontmatter round-trip property used `JSON.stringify` for the json leg because `@effected/jsonc` ships no public stringify — a surface-parity gap recorded for the jsonc package's roadmap, not this package's.
