...
As-built hardening (realignment, 2026-07-08 — two more defect surfaces closed, all covered by tests, compliance unaffected): (c) an **alias-expansion "billion laughs" bomb** that stayed under `maxAliasCount` (so the composer's alias-count guard never fired) but exhausted the heap during value materialization, escaping as an unhandled defect. `Yaml.parse`/`parseAll` value extraction now runs under an **alias-expansion budget derived from `maxAliasCount`**: an internal `AliasExpansionBudgetExceeded` throw is caught by the facade and materialized into a fatal `YamlParseError` carrying an `AliasCountExceeded` diagnostic. (d) **Deep-input stringify stack overflows** — `Yaml.stringify` (the plain-value path, `internal/stringifier.ts#stringifyLines`) and `YamlDocument.stringify` (the AST-node path, `stringifyNodeLines`) both overflowed the stack as `RangeError`/`Cause.Die` defects on deeply-nested acyclic input. Both recursion surfaces are now capped at the shared `MAX_NESTING_DEPTH = 256`; an internal `StringifyDepthExceeded` throw is caught by the facade and materialized into a fatal `YamlStringifyError` carrying a `NestingDepthExceeded` diagnostic. The node-path guard fires only on hand-built trees (composer output is already bounded to `MAX_NESTING_DEPTH`). With these two, the "malformed/hostile input fails typed, never a defect" invariant now holds on the alias-materialization and both stringify surfaces.
...
As-built (realignment, 2026-07-08): the `@public X_base` idiom described above — and every "`@public` base annotation" mechanics reference elsewhere in this doc (e.g. the [options derivation](#options-derivation) `YamlFormattingOptions` note) — is superseded by the inline factory form (see [effect-standards.md](../effect-standards.md#api-extractor--effect-class-factories)). All class factories are now written inline with no exported `*_base` const; the synthesized `_base` heritage symbols (including the co-recursive `YamlNode.ts` bases and the visitor-event union) are suppressed narrowly in `savvy.build.ts` (`ae-forgotten-export` / `_base` pattern) and land in the `issues.json` `suppressed` bucket, keeping it zero-warning. The `Schema.suspend` callbacks' own return-type annotations (`Schema.Schema<Self>`) survive where recursion requires them; the whole-class factory-return annotations do not. Genuinely-reusable public schemas (the staged code unions, literal sets that are real API) remain `@public` on their own merit; only the `*_base` symbols were removed.
...
- **All public `*Base` export pairs** — v3's doubled public error surface stays banned; the API-Extractor base need is met by the inline factory + narrow `_base` suppression (see the [API Extractor bases](#api-extractor-bases-house-policy) as-built note): no `*_base` symbol is exported at all.
