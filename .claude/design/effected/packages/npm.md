...
The vocabulary trigger fired: the [lockfiles importers work](lockfiles.md#importers-v2-addition-as-built-2026-07-14) is the second consumer (`ImporterDependency.specifier`), and the [workspaces snapshots work](workspaces.md#v2-additions-as-built-2026-07-15) is a third (`WorkspaceStateSnapshot.resolve` and the snapshot-scoped resolver layers). **`DependencySpecifier` moved here** on branch `feat/lockfiles-importers`. One specifier grammar now spans the kit — lockfiles, workspaces and package-json all classify a specifier the same way, instead of three prefix-sniffing reimplementations.
...
| `workspaces` | read, not modeled | workspaces reads globs (`internal/patterns.ts`) and — per [the v2 additions](workspaces.md#v2-additions-as-built-2026-07-15) — bun-style `catalog`/`catalogs`; package-json preserves the field via `rest`. Trigger for modeling: a consumer needing to *write* the field |
