...
**The test double takes an optional payload directly**, rather than routing it through an event-path filesystem read. The double hard-provides a noop filesystem, so seeding the path through overrides sends the read nowhere; without the argument, an action whose phase detection is a pure function of the payload has to hand-compose a filesystem stub at every call site. An unarranged payload still fails typed, naming the variable: the argument is additive and preserves the die-on-unstubbed posture rather than opening a silent-default escape hatch.
...
Beyond the toolkit-faithful accessors, two shapes consumers otherwise hand-roll are one implementation with one set of tests: a **list** accessor absorbing the whole union grammar consumers had reinvented (JSON arrays, bullet lists, comma-separated values) rather than shipping a variant per shape, and a **key-value pairs** accessor. A schema-decoding accessor covers everything else.
...
The installed provider's semantics are deliberately narrow, because a naive record-backed provider **uppercases every config path in the program**, silently changing the resolution of any `Config` a consumer wrote that is not an input:
...
**A latent defect worth not re-introducing:** an issue built with an absent actual value is classified as *missing data*, so **every validation failure is silently swallowed by any default** — `dry-run: yes` resolves to `false`, and the action performs every mutation the workflow author meant to rehearse. The issue therefore carries the offending value. Nothing about the default combinator's signature suggests the fallback depends on how the failure was *constructed*; this is a v4 semantics trap, not a typo.
...
**Both error sets are [per-reason tagged unions](github-actions.md#errors)** — see the `ActionOutputError` and `DetachedProcessError` aliases in `src/`. The aliases keep the surface stable; what the split buys is that "the runner file is not there" and "the output name is invalid" are separately recoverable, which is the distinction a `pre`/`post` phase actually acts on.
...
**The scan strips comments first**, for the phantom-edge reason the reachability walkers share: a raw text scan reads TSDoc as code, so a module whose comment *explains* that it does not unwrap a secret is reported as unwrapping one. It keeps a non-empty control and has a third test for the stripper — which is load-bearing once the scan depends on it, because a blinded scan is a silent false green.
...
Readiness polling is the poll-until-a-domain-predicate helper consumers otherwise hand-roll, usually with a suspension footgun.
