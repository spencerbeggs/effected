...
The whole package tests without `@effect/platform-node`: `Path.layer` and `FileSystem.layerNoop(partial)` come from `effect` core, so a stubbed filesystem drives discovery, enumeration and PM detection. Change-detection and snapshot tests stub `@effected/git`'s `Git` service with `Git.layerTest`, so they need no git repository. One integration test discovers *this repository* for real — the test that surfaces real-world file shapes (it is what surfaced the pnpm 11 config-dependencies lockfile-framing shape, now owned in `@effected/lockfiles`).
