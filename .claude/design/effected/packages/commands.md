...
It fills the [roadmap's reserved `@effected/commands` slot](../roadmap.md#effectedcommands) and is **Phase 1a** of the GitHub/Actions split program (`2026-07-25-github-split-master.md`).

The package is designed **against** core's `effect/unstable/process` vocabulary, never around it: a caller builds a `ChildProcess.Command` with core's constructors and combinators (`make`, `prefix`, `setCwd`, `setEnv`, `pipeTo`) and hands it to this package to run. One as-built caveat on that routing: bare `setEnv` produces a hermetic child environment (see the `extendEnv` note under the API surface) — `Run.extendEnv` is the path for adding variables on top of the parent environment. See [What this package deliberately does not do](#what-this-package-deliberately-does-not-do) — that boundary is the whole point of the package, and it is the correction of a recorded failure.
...
| `src/ScriptedSpawner.ts` | the public scripted `ChildProcessSpawner` double: `make`, `layer`, `spawns`/`SpawnRecord`, the `notFound` / `permissionDenied` statics ([as built](#as-built-the-public-scriptedspawner-test-double-2026-07-25)) |
...
- **`RunOptions` carries only what a core `Command` cannot.** `cwd`, `env`, `extendEnv`, `stdin`, `shell`, kill signals and fd wiring are all `ChildProcess.CommandOptions` fields with core combinators (`setCwd`, `setEnv`); duplicating them in `RunOptions` would be exactly the re-declaration REVERSAL 2 punished. A caller writes `ChildProcess.make("tar", args).pipe(ChildProcess.setCwd(dir))` — core's vocabulary, unchanged. What is left is genuinely ours: a deadline, a redaction set, a capture bound.
  ...
  **As-built (2026-07-25): the `setEnv`/`extendEnv` trap and `Run.extendEnv`.** Core's `setEnv` merges into `options.env` but never sets `extendEnv`, and the Node spawner resolves the child environment as `extendEnv ? { ...process.env, ...env } : env` — so bare `setEnv({ VAR: x })` spawns a child whose entire environment is that one variable (no `PATH`, no `HOME`), silently at the type level. `extendEnv` is publicly reachable only at construction. The fix is a **combinator, not a `RunOptions` field**: env shaping belongs on the `Command` value (the `RunOptions` rule above), and a field would be the re-declaration this bullet forbids. `Run.extendEnv` merges like `setEnv` (new values win), recurses into pipelines preserving `PipeOptions`, rebuilds via public `make`/`pipeTo`, and **deliberately forces `extendEnv: true`** even over a construction-time `false` — inheriting the parent env is its entire purpose; a hermetic env is what bare `setEnv`/construction options remain for. A unit CONTROL pins core's bare-`setEnv` behavior, and an e2e pair pins the platform env-resolution both ways.
...
The shape above has a second occupant `json` cannot serve: a child that **talks a protocol** rather than printing a document. `@effected/workspaces`' subprocess pnpmfile replay ([workspaces.md](workspaces.md#configdependencyhooks--the-opt-in-replay-seam)) runs foreign code that may `console.log` whatever it likes and may crash after reporting, and answers on a single final JSON line. `Run.json` is wrong for it twice over — it parses the *whole* of stdout, and it requires a zero exit — so the consumer hand-rolled last-line framing plus its own success/failure discrimination. `Run.jsonLine` is that framing, promoted: split stdout on `\r?\n`, drop whitespace-only lines, and — scanning from the **end** — take the first line that both `JSON.parse`s and decodes through the schema. (The original contract took the last non-empty line and required it to decode; #292 widened it after a consumer's pnpmfile hook logged from `process.on("exit", ...)` — *after* the payload had flushed — and won the last line. The widening is strict: any run whose last non-empty line decodes is byte-identical in outcome, and runs that previously failed on trailing noise now succeed.)

Two properties are the design, not conveniences:

- **It tolerates noise around the payload — before AND after.** The payload's position in the scan is the frame. A hook's own logging, a tool's warnings, an exit-hook's cleanup line — anything a child writes that does not decode under the schema is walked past rather than fatal, which is what makes the combinator usable against code the caller does not control. The consequence: if **multiple** lines decode under the schema, the **last** wins — a child must never emit two schema-valid lines, and a consumer envelope should be discriminated (a required `ok` literal, the `ReplayPayload` model) so an accidental log line cannot satisfy it.
...
When no line decodes anywhere the typed `CommandOutputError` carries the run's context (exit code, both streams, redacted) — see [Errors](#errors-two-both-structurally-routable) — and its `kind` keeps the near-miss diagnostic: `"schema"` with the **last JSON-parseable line's** decode failure as `cause` when at least one line parsed (that line being the most probable intended payload), `"notJson"` with the last non-empty line's parse error only when nothing anywhere was JSON. A spawn failure or an opted-in timeout still fails as `CommandFailedError`.
...
- **Signalling a bare pid later.** The reap happens in a *different process* (Actions `main` → `post`), so there is no handle — only a pid that travelled through `ActionState`. Core's kill is a method on a handle; there is no "signal an arbitrary pid" contract, and providing one means a `node:process.kill` import, which a boundary package may not have. **Home: `@effected/github-actions`** (integrated, `@effect/platform-node` as a required peer per the master plan's program decision #10) — where the pid's `ActionState` round-trip already lives, and where the `pid <= 0` process-group guard belongs beside the state that can produce a `-1`. That guard is load-bearing safety, not a detail: `kill(-1)` / `kill(0)` signal the caller's own process group and would take out the runner.
...
- **No `./testing` subpath, no behavior-reimplementing doubles.** `makeTest` / `layerTest` on the service, dying loudly on anything unstubbed. The public `ScriptedSpawner` (added 2026-07-25, below) is **not** an exception to this line or to the one rule: it reimplements no behavior and ships no backend — it *provides* core's own `ChildProcessSpawner` contract from a caller's script, which is the test-side analogue of `makeTest` on a service. The rule bans production implementations of core's subprocess concepts; a scripted double implements nothing for production.
...
- **Unit, mocked spawner.** `Layer.succeed(ChildProcessSpawner, ChildProcessSpawner.make(mockSpawn))` with `ChildProcessSpawner.makeHandle({...})` over in-memory streams — the `@effected/git` pattern, which needs no platform package. This carries the weight: exit-code routing, timeout, capture bound, redaction of both argv and captured output, `Run.json`'s three failure modes. **As built, nobody hand-rolls that**: the eleven-field handle is packaged once as the public `ScriptedSpawner` ([below](#as-built-the-public-scriptedspawner-test-double-2026-07-25)), which every unit suite here and downstream uses instead — `ScriptedSpawner.make(script).layer`, zero casts.
...
### As built: the public `ScriptedSpawner` test double (2026-07-25)

The downstream action rebuild's main friction ask, shipped as a seventh source module (`src/ScriptedSpawner.ts`). The package's guidance is "stub the spawner, not these" — `Run` is free functions, so the seam a consumer test replaces is core's `ChildProcessSpawner` — but actually scripting that contract by hand cost each consumer suite ~60 lines and casts, because `ChildProcessSpawner.makeHandle` wants eleven fields (`silk-update-action`'s hand-roll carried three `as never`s; `@effected/npm` carries a sibling hand-roll). The double packages that once, with **zero casts required by consumers** as the acceptance bar — met.

- **Shape.** `ScriptedSpawner.make(script)` returns an instance carrying `layer` (provides core's `ChildProcessSpawner`) and `spawns` (the live log). `script: (command, args) => ScriptResult` where `ScriptResult` is `{ stdout?, stderr?, exit?, hang? }` or a `PlatformError` that fails the spawn itself. `SpawnRecord` carries command, args, cwd/env/extendEnv (conveniences), the full `CommandOptions`, and `unrefed` — set only when the handle's `unref` effect actually RUNS, which is what lets consumer tests pin `Run.detach`'s ordering invariant. Statics `notFound` / `permissionDenied` build the two common spawn-failure `PlatformError`s (the shapes `ToolDiscovery`'s absence classification rests on). Static-class form: a private constructor and documented statics, `make` returning the instance.
- **Why it is not a rule violation.** The one rule bans *implementing* core's subprocess concepts for production — a backend, a runner service, a command type. A scripted double implements nothing for production: it provides core's own service, answering from the caller's script, exactly as `makeTest` provides a service's shape from stubs. It lives in `src/` as a free-standing named export (no `./testing` subpath), so a consumer that never imports it retains nothing.
- **`hang` support** carried over from the internal fixture: `hang: true` never resolves `exitCode`, exercising a caller-supplied timeout under `TestClock`.
- **Pipeline limitation (v1).** A piped command (`ChildProcess.pipeTo`) reaching the spawner **dies loudly** with a message naming the workaround (script each side separately, or go e2e against a real platform layer). Scripting a pipeline honestly means modeling core's `PipeOptions` routing, which no consumer has asked for; dying loudly beats a silently wrong answer.
- **The internal fixture is gone.** `__test__/fixtures.ts` was deleted and all three unit suites migrated — a straight rename (`scripted` → `ScriptedSpawner.make`, the error helpers → statics), since the public shape is a superset (adds the cwd/env/extendEnv conveniences). Being load-bearing machinery for every unit suite here and downstream, the double has its own direct suite (`__test__/ScriptedSpawner.test.ts`, 9 tests): script routing, the `{}` defaults, recording (options and `extendEnv`), record-on-run laziness, both spawn-failure classifications, hang + timeout, unref observation both ways, and the pipeline die.
