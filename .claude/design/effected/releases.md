...
The union of what those consumers need. Since 2026-07-14 the gate also names one new package in flight — `@effected/git`, [the point-in-time port](roadmap.md#5-the-point-in-time-port)'s new package. (A companion `@effected/commands` runner was on the gate for part of that day and was removed — core declares `ChildProcessSpawner`, platform layers provide it, and git requires it in `R`; the correction is recorded in [roadmap.md](roadmap.md#effectedcommands).) **All seventeen migrated library packages are merged** (`semver`, `jsonc`, `yaml`, `package-json`, `npm`, `config-file`, `walker`, `glob`, `toml`, `lockfiles`, `store`, `xdg`, `runtimes`, `runtime-resolver-cli`, `workspaces`, `ts-vfs`, `app`). `pnpm-plugin-effect` is outside that count because it is not a library — it is the kit's [companion](package-inventory.md#internal-packages-no-source-repo), published and installable but exposing no API ([effect-standards.md](effect-standards.md#companion-packages-published-but-not-a-library)). **It is on the gate all the same**: it ships at `0.1.0` with everything else. Being outside a count of libraries is not an exemption from the release.
...
| `@effected/workspaces` | integrated | merged | `vitest-agent`; takes `@pnpm/catalogs.*`, and implements `@effected/npm`'s resolver contracts. [The point-in-time port](roadmap.md#5-the-point-in-time-port) adds `silk-update-action` and `savvy-web/systems` (DepsRegen) as consumers, PM-aware catalog reads, the `WorkspaceSnapshots` service and the opt-in config-dependency hook replay; `GitReader` relocates to `@effected/git`, giving workspaces one new edge on `git` (boundary) — stays integrated |
...
| `@effected/git` | boundary | implemented (2026-07-14) | `GitCommand` values (core `Command`s) + the `Git` service requiring core's `ChildProcessSpawner` in `R` (consumer's platform layer provides it): typed read-only introspection (show, ls-tree, cat-file probe, merge-base, diff, rev-parse) plus `checkout`. `workspaces`' `GitReader` relocates here and dissolves — free only while nothing is published. Consumers: `workspaces` (`ChangeDetector`, `WorkspaceSnapshots`) and `savvy-web/systems`' DepsRegen directly. As-built: 44/44 tests, zero-warning `dist/prod/issues.json` (5 suppressed `_base` symbols). Design: [packages/git.md](packages/git.md) |
