...
| `@effected/app` | integrated | **implemented on `feat/app` 2026-07-12 — all gates green, pending merge** | the composition layer over `xdg` + `config-file` + `store` (integrated via R2 over `store`). 28 tests, `issues.json` 0/0/0. Design: [packages/app.md](packages/app.md). No consumer is blocked on it — nothing may depend on it |
