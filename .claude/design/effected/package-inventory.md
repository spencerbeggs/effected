...
| xdg-effect | @effected/xdg + @effected/store | boundary (xdg) + integrated (store) | store merged (migration #10); xdg in progress on `feat/xdg` | **Splits in two, decided 2026-07-09.** `@effected/xdg` keeps the XDG concepts only (`AppDirs`, `NativeDirs`, `XdgPaths`, `XdgSavePath`, the resolvers), expressed over `@effected/walker`. `@effected/store` takes the SQLite services. Its json-schema-effect dependency is a dead facade and gets cut |
...
Merged, fourteen packages: semver — jsonc — yaml — package-json (+ npm) — config-file (+ `-jsonc`, `-yaml`, and later `-toml`) — walker — glob — toml — lockfiles (migration #9) — store (migration #10).

Remaining, five packages:

1. **xdg** — after store, so it lands already slim. **In progress on `feat/xdg`.**
2. **workspaces** — after glob and lockfiles, both of which are merged.
3. **app-kit** — after xdg, config-file and store; it composes them and adds nothing else. Only xdg is still outstanding.
4. **type-registry** — load-bearing for two consuming apps, so not last.
5. **runtime-resolver** (+ CLI split) — depends only on semver, so it can move earlier if convenient.
