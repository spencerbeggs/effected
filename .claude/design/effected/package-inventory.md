...
Merged: 1. semver — 2. jsonc — 3. yaml — 4. package-json (+ npm) — 5. config-file (+ `-jsonc`, `-yaml`) — 6. walker — 7. glob — 8. toml — 9. config-file-toml.

Remaining, seven packages:

1. **lockfiles** — pure, no inbound blockers. **Implemented on `feat/lockfiles` (playbook steps 3–4 complete)** — 59 tests, zero-warning `dist/prod/issues.json` (8 suppressed `_base` symbols); moves to the merged list when the branch lands. Design: [packages/lockfiles.md](packages/lockfiles.md).
2. **store** — extracted from xdg; needed by type-registry and both external consumers.
3. **xdg** — after store, so it lands already slim.
4. **workspaces** — after glob and lockfiles.
5. **app-kit** — after xdg, config-file and store; it composes them and adds nothing else.
6. **type-registry** — load-bearing for two consuming apps, so not last.
7. **runtime-resolver** (+ CLI split) — depends only on semver, so it can move earlier if convenient.
