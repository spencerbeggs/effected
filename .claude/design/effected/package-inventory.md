...
Merged: 1. semver — 2. jsonc — 3. yaml — 4. package-json (+ npm) — 5. config-file (+ `-jsonc`, `-yaml`) — 6. walker.

Remaining, ten packages:

1. **glob** — first, while the minimatch dialect and CVE analysis are fresh (see [glob](#internal-packages-no-source-repo)). Its own spec → plan → implement cycle; its only consumer is `workspaces`, so it has room to land here.
...
5. **xdg** — after store, so it lands already slim.
6. **workspaces** — after glob and lockfiles.
