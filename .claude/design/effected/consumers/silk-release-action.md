...
| --- | --- | --- | --- |
| `src/utils/determine-tag-strategy.ts` | Single-vs-multi tag strategy from publishable packages + changeset config, plus `isMonorepoForTagging` | `VersioningStrategy.classify` / `.detect` and `strategy.tagsFor(releases, options?)` (`@effected/workspaces`) — collapses ~110 lines to two. **Warning (round-1 finding): `ReleaseTag` defaults unscoped names to a `v` prefix (`cli@v1.2.3`); Savvy convention needs `versionPrefix: ""` — no compile error signals the rename** | shipped |
