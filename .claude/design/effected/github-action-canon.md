...
`unit/` plus `integration/*.int.test.ts`. Rider: the collection contract is enforced executably and the gate is the `Tests:` line, never the exit code. Evidence: silk-release-action carried three coexisting test layouts, and `__test__/utils/*.test.ts` was silently never collected — a green exit code over tests that never ran. **The reserved helper-directory names are `utils`, `fixtures` and `snapshots`, and as measured against `@vitest-agent/plugin@2.5.5` on 2026-09-05 the exclusion reaches only `__test__`'s *direct children*.** A control probe proved the harness — `__test__/__probe_control__/` was collected — and against it `__test__/fixtures/` and `__test__/utils/` were skipped while `__test__/integration/fixtures/` and `__test__/integration/utils/` were **collected**. The mechanism is visible in the runner, and it is **not** one rule applied inconsistently — the two adjacent exclusions at `discover-strategy.js:122-123` cover different directory sets. `NON_DISCOVERABLE_DIRS` (`node_modules`, `.git`, `dist`) is excluded at **any depth**, where descending is always wrong; `TEST_HELPER_DIRS` (`fixtures`, `snapshots`, `utils`) is excluded only as a **direct child** of `__test__`. Those are ordinary directory names a suite may legitimately use at depth, so the narrow scoping reads as a deliberate trade rather than a missing glob — excluding them at any depth would silently swallow `__test__/integration/utils/thing.test.ts`. What is genuinely missing is documentation: "reserved directory" reads as position-independent, which is exactly the reading that produced the any-depth claim here. Filed as [spencerbeggs/vitest-agent#374](https://github.com/spencerbeggs/vitest-agent/issues/374), asking for the distinction to be stated rather than for the behaviour to change. **Do not depend on either answer**: name a nested helper directory something outside the reserved three (the mirror for `src/utils/` is `__test__/unit/utilities/`) and it is collected or skipped by placement rather than by a rule that may move. An earlier revision of this doc asserted the exclusion applied at any depth, citing silk-release-action #237 (5 suites, 60 cases lost); the loss was real and the direct-child rule explains it, but the any-depth generalization does not hold. A dated collection map has a shelf life — re-probe with a control rather than trusting it.
...
- **Gate before writing is now package mechanism, not repo discipline.** The generator is plain `SchemaPipeline.run(targets)` — nothing hand-rolled. The pipeline itself refuses a pinned versioned target's contract change before any write, failing `SchemaContractChangeError` and naming each blocked target's `version → nextVersion`; an unversioned or prerelease target rewrites in place because it has no consumer-pinning expectation to protect. The generator no longer needs its own `check`-then-abort preflight — that hand-rolled shape is exactly what silk-release-action's `generate-schema.ts` wrote for itself, and it over-blocked: it wedged on the action's own **unversioned** input schema, which has no predecessor for anyone to pin and no business being refused a rewrite. That is also why the pipeline's contract policy has no `"block"` value that refuses every contract change regardless of pinning — it would reproduce the same bug at the package level. (silk-release-action `lib/scripts/generate-schema.ts`, as the incident the mechanism now absorbs.)
- **The drift test imports the generator's own exported `targets`.** A test that rebuilds its own target list passes while the generator emits something else; asserting `SchemaPipeline.check` over the shared constant is the whole point. Assert `blocked` false, `contractBlocked` false and `DocumentDiff.isClean(change)` — the drift test reads `contractBlocked` alongside `blocked` so it prints the right remedy: "regenerate" is wrong advice for a target the generator itself will refuse to write; the fix there is a version bump. (silk-update-action `__test__/unit/generate-schema.test.ts`.)
- **Machine hints are a declared family, not description-only prose — this rider is falsified as written and now reads as follows.** `KeywordFamilies` declares `x-ai-` (with the trailing dash — bare `x-ai` and a look-alike prefix like `x-aida-foo` stay undeclared) as a house machine-annotation namespace alongside the upstream vscode/taplo/tombi/intellij families, and a key in it survives the Draft-07 lowering exactly as any other declared family does — since Effect rc.112 the lowering itself preserves unknown keywords, so there is no re-graft step and no carrier module to route through ([the section](packages/schemastore.md#annotation-carrying-core-carries-the-keywords-the-package-owns-admission)). The recommended, non-binding key is `x-ai-hint`, applied at the definition site like every other annotation in this package's model. Values must be JSON, and a declared-family value object must never carry an `$id` at any depth — ajv's reference collection walks unknown keywords looking for one, so a colliding `$id` nested inside an annotation payload fails the compile rather than no-opping. Every declared-family key must use ajv-legal key characters. The family is oriented at self-hosted publication: no upstream tool sanctions `x-ai-`, so a document carrying it that later goes to schemastore.org needs a corresponding entry in that repo's own validation config first. Both tracking issues land in this batch — [effected#598](https://github.com/spencerbeggs/effected/issues/598) (the family) and [effected#599](https://github.com/spencerbeggs/effected/issues/599) (generator scaffolding) are closed by it, not open promises.
...
- A default schemastore step: `lib/scripts/generate-schema.ts` over a small structured `result` output (replacing `summary-written`), a `schema:generate` script that runs it, and `schema:check` as the alias for the vitest drift test living under `__test__/unit/` (the exported `targets` constant, per B4 and its riders) — not a second script.
