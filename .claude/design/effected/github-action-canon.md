...
`savvy-web/github-action-template` is the worked example, regenerated to this specification with every verification gate green — including a runner-style smoke test of the built bundles, which is the only check that sees what `vitest` structurally cannot. Read the repository for the current scaffold; this section holds only what a reader of the canon needs.
