...
`plugin/` houses the "effected" Claude Code plugin for Effect v4 development. It ships a catalog of 28 skills, four specialist subagents and a SessionStart briefing hook, and is dogfooded during package work (see [migration-playbook.md](migration-playbook.md)). It is [repo infrastructure, not an `@effected` library](architecture.md), and during dogfooding it is loaded via `claude --plugin-dir plugin` (the root `package.json` `claude` script).
...
- `agent-skill-registration.bats` — each agent's frontmatter `skills` list, including the assertion that `action-engineer` lists `designing-an-action`, so the fix for the orphaned-skill finding cannot be silently reverted by a frontmatter edit. It also catches the specific bug of a skill name leaking into an agent's `tools` block, which would still satisfy any test that merely greps the file for the name.
