...
Every internal `@effected/*` edge — peer and regular dependency alike — uses `workspace:^` (changed from `workspace:~` on 2026-08-05; the paired `devDependency` that satisfies an auto-installed peer stays `workspace:*`, which is not published). That is patch-floating: a sibling patch flows into an existing release without forcing a coordinated re-release ([issue #133](https://github.com/spencerbeggs/effected/issues/133)), while a minor bump still needs a coordinated release. Whether an edge is a peer or a regular dependency is still decided per edge at design time — that choice is about the shared-instance contract, not the version range. Floating the regular edges too, not only the peers, lets a consumer's paths dedupe onto one sibling copy, which matters where an integrated package surfaces a sibling's types across its API.

**`^` and `~` are the same range for every package in this kit today, and that is why the change was safe.** At `0.x.y` with a non-zero minor — which is every one of the 27 packages — the caret's "don't change the left-most non-zero digit" rule and the tilde's "patches only" rule both resolve to `>=x.y.z <x.(y+1).0`. Verified with the kit's own `Range.equivalent(^0.3.1, ~0.3.1) === true`. So the switch altered no resolution, and **it cannot have been what fixed a peer-resolution problem** — a diagnosis that attributes such a fix to the operator change is looking at the wrong cause. (The one `0.x` shape where they differ is `0.0.z`, where `~` is the *looser* of the two. Nothing here sits at `0.0.z`.)

**The decision it actually makes is the `1.0.0` one.** At `1.x`, `^1.2.3` admits every later `1.x` and `~1.2.3` admits only patches. `^` is the correct choice there, and for a reason specific to this kit rather than general taste: the paragraph above wants a consumer's paths to **dedupe onto one sibling copy**, and range width is what makes that possible. Two consumers requiring `^1.2.0` and `^1.5.0` both resolve to a single `1.5.x`; under `~1.2.0` and `~1.5.0` they cannot, and the tree gets two copies of one package. In this kit two resolved copies are two distinct `Context.Service` tags, which surfaces as a service reading unprovided in a graph that visibly provides it — not as a version error. The wider range is the safer one here, which inverts the usual intuition.
