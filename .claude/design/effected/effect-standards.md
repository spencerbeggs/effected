...
4. `pnpm-workspace.yaml` sets `dedupeDirectDeps: false`. It no longer pins `dedupePeerDependents: false` — that setting was removed once the underlying peer-poisoning was fixed, so pnpm's default applies. **Corrected 2026-07-09:** this document previously asserted the `dedupePeerDependents` pin as a live requirement; the key is not present anywhere in the repo, and neither is an `.npmrc`. Read `pnpm-workspace.yaml` rather than trusting a doc for this.
