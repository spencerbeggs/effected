...
//
// Usage:
//   node plugin/scripts/generate-constructs.mts generate [--packages DIR] [--out DIR] [--annotations FILE]
//   node plugin/scripts/generate-constructs.mts check    [--packages DIR] [--annotations FILE] [--require-intent]
//
	...
	if (command === "check") {
		...
		if (flags.get("require-intent") === true) {
			...
			if (unannotated > 0) {
				...
				console.error("Author them in plugin/scripts/construct-annotations.json (see .claude/skills/constructs).");
				failed = true;
			}
		}
