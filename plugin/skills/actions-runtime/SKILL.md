...
### The `isDebug` → `MinimumLogLevel` bridge — boilerplate every action repeats

`ActionEnvironment.isDebug` (`Effect.Effect<boolean>`, reading
`RUNNER_DEBUG === "1"`) only answers *whether* step debugging is on — it does
not, by itself, make `Effect.logDebug` calls visible. Core's
`References.MinimumLogLevel` defaults to `"Info"`
(`.repos/effect/packages/effect/src/References.ts:433`, default stated in its
own TSDoc example), so a `Debug`-level log is filtered before
`ActionLogger.logger` ever gets a chance to render it as `::debug::` — the
level split `actions-reporting` documents only fires for entries that survive
this filter first. Wiring the two together is a few lines every action
currently reinvents:
...
minimum level is a run-wide setting, not a per-step decision.
