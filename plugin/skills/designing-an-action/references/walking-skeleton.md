...
One module per pipeline step, each exporting: a result type, a typed error (plus a stored message and an optional cause), a declared requirement channel, and a stub that always succeeds with a documented placeholder value.

**Two error shapes are both correct; pick by a test, not by habit.** A single tagged class with a closed `reason` union is right when every reason carries the SAME fields — it keeps one name and one handler. Split into one class per failure, exported as a union type alias, when either holds: the reasons carry DIFFERENT fields, or a caller wants to recover from one failure without catching the rest. The first is the stronger signal — a `reason` union whose members need different data ends up with every field optional, and then a message interpolates a field its own reason requires and renders `undefined` at the exact moment someone is debugging. The kit moved four of its own errors this way for that reason; see `@effected/github-actions`'s `ActionOutputs.ts` for the argument in source. `Effect.catchTag` recovering one arm rather than all of them is the second signal's payoff. Stubs never fail — a failing stub at this stage is a sign the skeleton discipline is being skipped, not a sign of a real bug being found early.
