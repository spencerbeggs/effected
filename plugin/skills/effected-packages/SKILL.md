...
| `@effected/schemastore` | Effect Schemas published as SchemaStore-shaped Draft-07 JSON Schema documents: `StoreDocument` assembly (owning the `#/$defs` restore), versioned/unversioned catalog modes, fileMatch hygiene lint, `DocumentLint`, language-server annotation carriers, a `SchemaValidator` seam the consumer closes with ajv, `SchemaFile` write-if-changed IO | emitting editor-consumable JSON Schema from Effect Schemas, or publishing a catalog to SchemaStore | boundary | the package's own `CLAUDE.md` |
