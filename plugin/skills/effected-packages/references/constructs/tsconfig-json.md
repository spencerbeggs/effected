...
| --- | --- | --- | --- |
| `CompilerOptions` | Variable + Namespace | Type-only companion namespace for {@link (CompilerOptions:variable)}, exposing its decoded and encoded shapes. | |
| `EnumFamily` | TypeAlias | The nine `compilerOptions` / `watchOptions` enum families this codec knows, per R1.6. | |
| `FallbackPolling` | Variable | `watchOptions.fallbackPolling`. | |
...
| `ModuleResolution` | Variable | `compilerOptions.moduleResolution`. `node10`, `node` and `classic` are deprecated in TS 6.0. | |
| `NewLine` | Variable | `compilerOptions.newLine`. | |
| `PortableTsconfig` | Class + Interface | The portable-tsconfig filter: {@link (PortableTsconfig:class).make} narrows a resolved or bare compiler-options object to the allow-listed, machine-independent subset described on {@link (PortableTsconfig:interface)}. | |
...
| `Reference` | Variable + Namespace | Type-only companion namespace for {@link (Reference:variable)}. | |
| `ResolvedTsconfig` | Class + Interface | The pure extends-merge engine: parse-time path absolutization ({@link (ResolvedTsconfig:class).absolutize}), the per-field merge fold ({@link (ResolvedTsconfig:class).merge}), and the `${configDir}` final phase ({@link (ResolvedTsconfig:class).substituteConfigDir}). | |
...
| `Target` | Variable | `compilerOptions.target` — the ECMAScript target. `es5` is deprecated in TS 6.0; `es3` has no literal (dead per R1.3 — a `target: "es3"` value fails decode against this schema rather than silently passing through, since `target` itself is a live, typed field). | |
...
| `WatchOptions` | Variable + Namespace | Type-only companion namespace for {@link (WatchOptions:variable)}. | |
