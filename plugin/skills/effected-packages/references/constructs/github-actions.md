...
| `DryRunShape` | Interface | The `DryRun` service shape. | |
...
| `GitHubCacheBlobStore` | Class | The `BlobStore` backend that stores blobs in the runner's own Actions cache. | BlobStore backend over the runner's own Actions cache, no S3 bucket needed |
...
| `GitHubListOptions` | Interface | Options for `GitHubMarkdown.list`. | |
...
| `GitHubRowSchema` | TypeAlias | The row-schema constraint `GitHubMarkdown.tableFor` accepts: any schema exposing a struct field map — `Schema.Struct` and `Schema.Class` both qualify. | |
| `GitHubSchemaTable` | Interface | A renderer minted by `GitHubMarkdown.tableFor`: columns are fixed by the schema, rows are supplied at each render. | |
| `GitHubSchemaTableColumn` | Interface | Per-column configuration for `GitHubMarkdown.tableFor`. | |
| `GitHubSchemaTableColumns` | TypeAlias | The per-field column map for `GitHubMarkdown.tableFor`: entries are optional except where the field's encoded side is not a string, where the entry — and its `format` — are required. | |
...
| `GitHubSchemaTableFormattedColumn` | Interface | `GitHubSchemaTableColumn` with the `format` obligation discharged — the entry shape a non-string-encoded field demands. | |
| `GitHubSchemaTableOptions` | TypeAlias | Options for `GitHubMarkdown.tableFor`. The whole bag is optional exactly when every field's encoded side is a string; one field without a string projection makes `columns` — and the entry carrying its `format` — required. | |
...
| `InstalledPackageManager` | Variable + TypeAlias | The decoded type of `(InstalledPackageManager:variable)`: `AmbientPackageManager` `\|` `CachedPackageManager`. | decoded union of an ambient or tool-cache package manager install result |
...
| `InvalidPidError` | Class | Raised when a non-positive pid was handed to `DetachedProcess.reap`. | handle a non-positive pid refused before signaling a process group by mistake |
...
| `ProvisionFileOptions` | Interface | What `ToolInstallerShape.provisionFile` should provision. | |
