...
| `PackageStateSnapshot` | Class | One workspace member as captured in a {@link WorkspaceStateSnapshot} — the serializable slice a snapshot diff reads: identity, version, location, and the four dependency records. | one package's captured state at a point in time: name, version, dependencies |
