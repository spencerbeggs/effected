...
| `Git` | Class | Typed git introspection over core's `ChildProcessSpawner`: read a repository's state at any ref without checking it out (including the network read `lsRemote` and the index read `lsFiles`), plus the mutating tier — checkout/fetch, the working-tree restore trio and stash, branches and tags, remotes, worktrees, commit/push/pull, submodules, sparse-checkout, config writes and staging — that changes it. Every mutating method's TSDoc opens with the literal word `Mutating:`. | typed git introspection and mutation service, run git commands with redacted argv, read a file at a ref, check if a ref exists, list branches tags refs, git status, git commit push pull, submodule commands |
| `GitCommand` | Class | Pure constructors for the `git` {@link GitInvocation} values this package spawns. | pure constructors for git argv/invocations with redacted secrets, build a git command with redacted argv |
