...
| --- | --- | --- | --- |
| `BareLineReference` | Interface | The closing reference a bare line carries, per `parseBareLineReference`. | |
...
| `ClosingList` | Interface | The issues a closing-list line names, per `parseClosingList`. | |
| `HarvestedReferenceList` | Interface | A reference list harvested out of running text by `harvestReferenceLists`, with the offsets a prose harvest needs. | |
| `IssueReference` | Interface | One closing reference found in prose by `harvestIssueReferences`. | |
