...
**The sync facade is also the tolerant-enumeration path, and that is a second, independent reason to choose it.** The deciding question is often error semantics rather than sync-vs-async: `WorkspaceDiscovery.listPackages()` fails typed on ANY unusable member, and a version-less root `package.json` (`{"name":"x","private":true}` with no `version`) is an ordinary pnpm shape that trips it — so one unusable member degrades the whole listing to a failure. The sync facade is documented total and skips unusable members instead. When you want skip-don't-fail enumeration, `getWorkspacePackagesSync` is the surface that gives it to you, whether or not an Effect runtime is available.
