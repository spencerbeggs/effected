...
- **`resolveEntryPoint(manifest, options?)`** — which file is a package's `"."` entry, as a **pure, IO-free** function over any object carrying `exports`/`main`. Returns `Result<string, UnresolvedEntryPointError>`. Honours all three legal `exports` spellings (string shorthand, subpath map, root conditions with no `"."` key) and recurses through nested conditions; `options.conditions` defaults to `["import", "default"]` and its ORDER is the policy. **`exports` encapsulates the package**: present-but-unmatched answers a typed failure and does **not** fall through to `main` — that is Node's rule, and the lenient reading resolves a file the package deliberately does not export, which then loads and behaves plausibly instead of failing. Do not "fix" it back to lenient. The failure `reason` is discriminated (`noRootExport | noConditionMatched | unsupportedExportsForm`) so a caller can log which shape a package actually had. Pair it with `@effected/npm`'s `PackageTarball` to read an entry out of a published tarball.
