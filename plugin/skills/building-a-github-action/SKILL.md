...
- **Errors are `Schema.TaggedErrorClass` with a `kind` discriminant and ergonomic statics**, and every error reason has a test that fires it. When porting a channel, demonstrate its failure path or delete it from the signature.
...
- No lowercase-h markdown-writer spelling — the GFM writer is `GitHubMarkdown` (`table`, `tableFor(schema)`, `heading`, `link`, `code`, `codeBlock`, `list`, `details`, `raw`).
