...
3. **Probe files live at the package root** — *inside* `packages/<pkg>/`, written there, not merely run from there. Two distinct failures, and they bite at different moments:
   ...
   **Exception — probing a package's OWN engine/internal modules: use `npx tsx`.** When the probe's entry imports the package's source files by path (not bare `effect` imports), `npx tsx /tmp/probe.ts` is the better vehicle: tsx resolves the house `.js`-extension TypeScript imports correctly, transitive bare imports resolve from the *imported file's* location (so the package's own `effect` still wins), and the probe lives outside the tree entirely — it cannot poison a test project. Never park a scratch `*.test.ts` in `__test__/` to probe: one file with a load-time error silently zeroes the package's whole run (`Tests: 0/0 passed`, exit 0 — the false green in `effect-v4-testing`). For bare-`effect` probes, the package-root rule above still stands.
