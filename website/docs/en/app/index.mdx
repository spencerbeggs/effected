...
All three `filename` options take a **single path component**. An empty name, one containing a separator (`/` or `\`), or a bare `.`/`..` would resolve outside the intended file within the namespace directory, so it dies at layer construction — it can only come from code, never from user input.
