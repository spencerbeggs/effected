...
Both `filename` options take a **single path component**. An empty name, or one containing a separator, would escape the namespace directory, so it dies at layer construction — it can only come from code, never from user input.
